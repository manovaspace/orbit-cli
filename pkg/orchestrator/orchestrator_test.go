...
func createTestRepo(t *testing.T, dir string, branch string) {
	...
	runGitCmd(t, dir, "config", "core.hooksPath", "")
}
...
func TestCloneTarget(t *testing.T) {
	...
	// 6. Non-empty directory without .git
	nonEmptyDir := filepath.Join(workspaceRoot, "existing-dir")
	_ = os.MkdirAll(nonEmptyDir, 0755)
	_ = os.WriteFile(filepath.Join(nonEmptyDir, "code.go"), []byte("package main\n"), 0644)
	resNonEmpty := CloneTarget(workspaceRoot, manifest.RepoTarget{
		Name:      "existing-dir",
		Path:      "existing-dir",
		RemoteURL: bareDir,
	})

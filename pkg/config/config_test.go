...
func TestLegacyMigration(t *testing.T) {
	t.Run("Migrate ~/.config/manova/config.yaml on Load(\"\")", func(t *testing.T) {
		tmpHome := t.TempDir()
		t.Setenv("HOME", tmpHome)
		t.Setenv("XDG_CONFIG_HOME", "")
		t.Setenv("ORBIT_CONFIG", "")

		legacyDir := filepath.Join(tmpHome, ".config", "manova")
...
`
		...
		if cfg.Server.URL != "https://legacy.manova.space" {
			t.Errorf("expected server url https://legacy.manova.space, got %s", cfg.Server.URL)
		}
		if cfg.Staff.URL != "https://legacy-staff.manova.space" {
			t.Errorf("expected staff url https://legacy-staff.manova.space, got %s", cfg.Staff.URL)
		}

		// Verify target file now exists and has 0600 mode
		fi, err := os.Stat(targetFile)
		if err != nil {
			t.Fatalf("target file was not created: %v", err)
		}
		if perm := fi.Mode().Perm(); perm != 0600 {
			t.Errorf("expected target file mode 0600, got %o", perm)
		}

		// Verify directory has 0700 mode
		dirFi, err := os.Stat(filepath.Dir(targetFile))
		if err != nil {
			t.Fatalf("target dir stat failed: %v", err)
		}
		if perm := dirFi.Mode().Perm(); perm != 0700 {
			t.Errorf("expected target dir mode 0700, got %o", perm)
		}
	})

	t.Run("Migrate ~/.config/manova/config.yaml on Resolve(ResolveOptions{})", func(t *testing.T) {
		tmpHome := t.TempDir()
		t.Setenv("HOME", tmpHome)
		t.Setenv("XDG_CONFIG_HOME", "")
		t.Setenv("ORBIT_CONFIG", "")
  ...
  url: https://legacy-resolve.manova.space
`
		if err := os.WriteFile(legacyFile, []byte(legacyContent), 0644); err != nil {
			t.Fatalf("WriteFile legacy failed: %v", err)
		}
		...
		if res.Config.Server.URL != "https://legacy-resolve.manova.space" {
			t.Errorf("expected server url https://legacy-resolve.manova.space, got %s", res.Config.Server.URL)
		}
	...
	t.Run("Migrate with XDG_CONFIG_HOME set", func(t *testing.T) {
		tmpDir := t.TempDir()
		t.Setenv("XDG_CONFIG_HOME", tmpDir)
		t.Setenv("ORBIT_CONFIG", "")

		legacyDir := filepath.Join(tmpDir, "manova")
		if err := os.MkdirAll(legacyDir, 0755); err != nil {
			t.Fatalf("MkdirAll failed: %v", err)
		}
		legacyFile := filepath.Join(legacyDir, "config.yaml")
		legacyContent := `server:
  url: https://xdg-legacy.manova.space
`
		if err := os.WriteFile(legacyFile, []byte(legacyContent), 0644); err != nil {
			t.Fatalf("WriteFile legacy failed: %v", err)
		}
		...
		if cfg.Server.URL != "https://xdg-legacy.manova.space" {
			t.Errorf("expected server url https://xdg-legacy.manova.space, got %s", cfg.Server.URL)
		}

		targetFile := filepath.Join(tmpDir, "orbit", "config.yaml")
		fi, err := os.Stat(targetFile)
		if err != nil {
			t.Fatalf("target file was not created: %v", err)
		}
	...
	t.Run("Existing target file is not overwritten by legacy file", func(t *testing.T) {
		tmpHome := t.TempDir()
		t.Setenv("HOME", tmpHome)
		t.Setenv("XDG_CONFIG_HOME", "")
		t.Setenv("ORBIT_CONFIG", "")
		...
		_ = os.WriteFile(filepath.Join(legacyDir, "config.yaml"), []byte("server:\n  url: https://legacy.space\n"), 0644)

		targetDir := filepath.Join(tmpHome, ".config", "orbit")
		_ = os.MkdirAll(targetDir, 0700)
		targetFile := filepath.Join(targetDir, "config.yaml")
		_ = os.WriteFile(targetFile, []byte("server:\n  url: https://orbit-existing.space\n"), 0600)

		cfg, err := Load("")
		if err != nil {
			t.Fatalf("Load failed: %v", err)
		}
		if cfg.Server.URL != "https://orbit-existing.space" {
			t.Errorf("expected existing url https://orbit-existing.space to be preserved, got %s", cfg.Server.URL)
		}
	})
}

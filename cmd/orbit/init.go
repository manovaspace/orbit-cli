  ...
  <repo-name>     - Clones an individual repository`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if zshRes.Status != doctor.StatusOK || omzRes.Status != doctor.StatusOK || shellRes.Status != doctor.StatusOK {
				fmt.Fprintf(out, "\n  %s  %s\n", iconWarn, warningStyle.Render("Zsh, Oh My Zsh, and Zsh default shell are required for Orbit developer workspaces."))
				if promptYesNo(in, out, "Would you like to install and configure Zsh + Oh My Zsh now?", true) {
					...
					_, _ = alias.AddShellAlias("o", "orbit")
				} else {
					return fmt.Errorf("zsh and oh-my-zsh are mandatory for Orbit workspace development; setup aborted")
				}
			} else {
				if home, err := os.UserHomeDir(); err == nil {
					_ = doctor.EnsureZshConfigured(home)
			...
			if bootstrapFlag {
				fmt.Fprintf(out, "\n%s\n", successStyle.Render(fmt.Sprintf("✔ Orbit developer environment initialized (%s)!", version)))
				fmt.Fprintf(out, "  • Zsh and Oh My Zsh configured as default shell.\n")
				fmt.Fprintf(out, "  • 'o' alias and shell completions active in ~/.zshrc.\n")
				fmt.Fprintf(out, "  • Unix man pages installed (run 'man orbit' or 'man o').\n\n")
				fmt.Fprintf(out, "When you receive an invitation token, run '%s' to claim your workspace.\n\n", boldStyle.Render("o onboard"))
				return nil
			}

...
func newChangelogCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "changelog",
		Short: "View recent release notes and what's new in Orbit",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if targetVersion != "" {
				...
				content := fmt.Sprintf("%s\n\n%s\n",
					titleStyle.Render(fmt.Sprintf("Orbit Release Notes — %s", rel.Version)),
			...
			recent := changelog.GetRecentReleases(releases, limit)
			content := fmt.Sprintf("%s\n\n%s\n",
				titleStyle.Render("Orbit Changelog & Release Notes"),
				changelog.FormatAllCards(recent),
			)

			if len(recent) < len(releases) {
				tip := fmt.Sprintf("\n💡 Tip: Showing %d latest releases. Run 'o changelog -n 10' for more, or 'o changelog -v <tag>' for a specific version.", len(recent))

...
func TestUninstallHelp(t *testing.T) {
	...
	output := buf.String()
	if !strings.Contains(output, "Uninstall the orbit CLI binary") {
...
func TestUninstallWithYesAndKeepConfig(t *testing.T) {
	...
	fakeConfig := filepath.Join(tmpHome, ".orbit")
	...
	if !strings.Contains(outStr, "Successfully uninstalled orbit CLI") {
		t.Errorf("expected uninstalled confirmation in output, got: %s", outStr)
	}
	...
	// Edge state files should be removed even with --keep-config
...
func TestUninstallFullPurgeCleansConfig(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	...
	// Full purge should delete entire .orbit directory
	if _, err := os.Stat(fakeConfig); !os.IsNotExist(err) {
		t.Errorf("expected ~/.orbit to be deleted during full uninstall")
	}
}

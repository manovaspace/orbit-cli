...
func newSelfUpdateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:    "Checks Forgejo/GitHub releases for the latest orbit CLI binary, downloads matching OS/arch archive, and replaces the running binary.",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if targetVersion != "" {
				...
				if checkOnly {
					fmt.Fprintf(out, "Run 'orbit self-update --version %s' without --check to install.\n", targetVersion)
					return nil
				}
				...
				fmt.Fprintf(out, "  %s  %s\n", iconOK, successStyle.Render(fmt.Sprintf("Successfully updated orbit to %s!", targetVersion)))
			...
			fmt.Fprintf(out, "  %s  %s\n", iconOK, successStyle.Render(fmt.Sprintf("Successfully updated orbit to %s!", updater.FormatVersion(res.LatestVersion))))

			// Run Post-Update Environment Migrations (systemd worker, completions, o alias prompt)
			execPath, _ := os.Executable()
			postCtx := &migrate.PostUpdateContext{
				Interactive: true,
				In:          in,
	...
	cmd.Flags().StringVarP(&targetVersion, "version", "v", "", "Target specific release version (e.g. v0.1.0)")

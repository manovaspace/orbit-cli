...
func newMigrateCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "Executes pending workspace migrations (directory structure, git hooks, Cursor rules, MCP configs) and tracks applied state in .orbit/state.json.",
...
func newMigrateStatusCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(out, "  State File: %s\n\n", subtleStyle.Render(workspaceRoot+"/.orbit/state.json"))

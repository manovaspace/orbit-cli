...
func TestInviteCreateCmd(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "Manova Developer Invitation Generated") {
	...
	if !strings.Contains(out, "manova onboard --token manova-inv.") {
...
func TestInviteListCmd(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "Manova Developer Invitations") {

...
const (
	DefaultSecretEnv      = "MANOVA_INVITE_SECRET"
	DefaultFallbackSecret = "manova-dev-insecure-invitation-signing-secret-key-32bytes"
)
...
func newInviteCreateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			out := cmd.OutOrStdout()
			fmt.Fprintln(out, titleStyle.Render("Manova Developer Invitation Generated"))
			...
			instructions := fmt.Sprintf(
				"To onboard with this invitation token, run:\n\n"+
					...
					"Or using NPX:\n\n"+
					"  %s",
				codeStyle.Render("manova onboard --token "+tokenStr),
				codeStyle.Render("npx @manovaspace/cli onboard --token "+tokenStr),
			)
...
func newInviteListCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(records) == 0 {
				fmt.Fprintln(out, infoStyle.Render("No invitations found. Run 'manova invite create <email>' to generate one."))
				return nil
			}

			fmt.Fprintln(out, titleStyle.Render("Manova Developer Invitations"))
